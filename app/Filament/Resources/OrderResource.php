...
class OrderResource extends Resource
{
    ...
    {
        return static::getModel()::query()->where('status', 0)->where('callc', 0)->orderBy('id', 'desc');
    }
    public static function getNavigationBadge(): ?string
    {
        return count(Order::where('status', 0)->where('callc', 0)->get());
    }
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->filters([
                ...
                // SelectFilter::make('zone')->relationship('Zone', 'AddresAr')->columnSpan(3),
                // SelectFilter::make('Branch')->relationship('Branch', 'AddresAr')->columnSpan(2),
                    ...
                    ->query(function (Builder $query, array $data): Builder {
                        return $query
                            ...
                            ->when(
                                ...
                                $data['del_from'],
                                fn(Builder $query, $date): Builder => $query->whereDate('DeliveryTime', '>=', $date),
                            )
                            ->when(
                                $data['del_until'],
                                fn(Builder $query, $date): Builder => $query->whereDate('DeliveryTime', '<=', $date),
